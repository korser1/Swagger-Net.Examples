...
This is a fork from Swashbuckle.Examples to use it with Swagger-Net
A simple library which adds the `[SwaggerRequestExample]`, `[SwaggerResponseExample]` attributes to [Swagger-Net](https://github.com/domaindrivendev/swashbuckle). Can also add read the `[Description]` attributes off your Response objects, and can also add an input box for entering an Authorization header.
...
| Version of Swagger-Net you're using | You'll want this version of this package |
|-------------------------------------|-----------------------------------------|
| Swagger-Net 1.0 | https://www.nuget.org/packages/Swagger-Net/ |
...
## Installation
Install the [NuGet package](https://www.nuget.org/packages/Swagger-Net.Examples/), then enable whichever filters 
...
swagger.json, **only one example for responses will display on the swagger-ui page**. This is due to a bug in swagger-ui. [Issue 9](https://github.com/mattfrear/Swagger.Net.AspNetCore.Examples/issues/9) You may want to use Swagger.Net's  `[SwaggerResponseRemoveDefaults]` attribute to remove 200 from your list of response codes, if for example your API returns 201s and not 200s [Issue 26](https://github.com/mattfrear/Swagger.Net.AspNetCore.Examples/issues/26).
N.B. this issue is fixed in Swagger.Net.AspNetCore.
...
why it is being displayed incorrectly - I suspect it's a bug in swagger-ui, as this didn't happen with older versions of Swagger.Net.

- Response examples which are only a simple type, i.e. string, int, etc and not an object may cause the swagger-ui page to stop rendering correctly. See [Issue 25](https://github.com/mattfrear/Swagger.Net.Examples/issues/25) for a possible workaround.
